...
export function DeckForm({ initialValues, onSubmit, onCancel, submitLabel = "Créer" }: DeckFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {error && (
        <div className="rounded-[1rem] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">{error}</div>
      )}
      ...
      <Input
        ...
        placeholder="français, vocabulaire, A2"
      />
      ...
      <Input
        ...
        placeholder="Langues, Sciences, etc."
      />
          ...
          {(["private", "public", "link_only"] as const).map((v) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                visibility === v
                  ? "border border-[#54462f] bg-[linear-gradient(140deg,#655438,#4f422e)] text-[#fff9ef]"
                  : "border border-[#d4cab8] bg-white text-[#585042] hover:border-[#b9aa90]"
              }`}
            >
